...
class Alumno extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('alu_rut', 'validateRut'),
			array('alu_rut', 'required', 'message' => '<span class="required">Este campo es requerido</span>'),
			array('alu_rut, alu_informe_curricular', 'length', 'min'=>10),
			array('alu_rut','match','pattern'=>'[^0*(\d{1,3}(\.?\d{3})*)\-?([\dkK])$]'),
			
			
			
			array('pro_rut', 'required', 'message' => '<span class="required">Este campo es requerido</span>'),
			array('pro_rut, alu_informe_curricular', 'length', 'min'=>10),
			array('jef_rut', 'required', 'message' => '<span class="required">Este campo es requerido</span>'),
			array('jef_rut, alu_informe_curricular', 'length', 'min'=>10),			
			array('alu_clave, alu_informe_curricular', 'length', 'max'=>50),
			array('alu_nombre', 'required', 'message' => '<span class="required">Este campo es requerido</span>'), 
			array('alu_apellido_paterno', 'required', 'message' => '<span class="required">Este campo es requerido</span>'), 
			array('alu_apellido_materno', 'required', 'message' => '<span class="required">Este campo es requerido</span>'),
			array('alu_email', 'email', 'message' => '<span class="required">El correo ingresado no es un correo valido</span>'),
			array('alu_direccion, alu_carrera', 'length', 'max'=>30),
...
public function validateRut($attribute, $params) {
	...
	if ($verifyCode != $result) $this->addError('alu_rut', '<span class="required">Rut invalido.</span>'); 
